with source as (
    ...
    select *, 'regular_season' as game_type from {{ source('retrosheet', 'regular_season_games') }}
        
    union all
    
    select *, 'all_star_game' as game_type from {{ source('retrosheet', 'all_star_games') }}
    
    union all 
    
    select *, 'division_series' as game_type from {{ source('retrosheet', 'division_series_games') }}
    
    union all 
    
    select *, 'league_championship_series' as game_type from {{ source('retrosheet', 'league_championship_games') }}
    
    union all 
    
    select *, 'wild_card_series' as game_type from {{ source('retrosheet', 'wild_card_games') }}
    
    union all 
    
    select *, 'world_series' as game_type from {{ source('retrosheet', 'world_series_games') }}
