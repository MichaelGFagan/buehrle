with source as (
    ...
    select * from {{ source('fangraphs', 'pitching') }}
...
transformed as (
    ...
    select
      ...
      , round(cast(trim(source.dollars, '()$') as numeric), 1) as wins_above_replacement_dollar_value
...
final as (
    ...
    select
        {{ dbt_utils.generate_surrogate_key(['person_id', 'year_id']) }} as player_year_id
      , *
