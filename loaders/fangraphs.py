...
parser.add_argument('-e', '--end', type=int, default=2024, help="last year to extract from Fangraphs")
...
PATH = os.path.dirname(__file__).replace('loaders', 'data/fangraphs')
...
def fangraphs(start_year, end_year):
    for year in range(start_year, end_year):
        for side in ['batting', 'pitching']:
            ...
            df = stats(year, qual=0)
            df.rename(columns=RENAMED_COLUMNS, inplace=True)
            df.to_csv(f'{PATH}/{side}/{filename}', index=False)
